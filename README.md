...
Already chatting with Claude Code? Skip the terminal: paste this in and it'll explain the two options below, ask which you want, then install it.
...
Installs the statusline, then walks you through changes in plain English: see your current bar, browse every field and style, preview live, write only when you're happy. Best if you want to tweak the design.

→ see [Using the skill](#using-the-skill) below for how to customize it.

### Option 2: one-liner (default only, fast)
...
→ see [Using the skill](#using-the-skill) below: you can add it anytime, the default doesn't lock you out.

**Requirements:** `jq` (recommended) or `python3`. The script falls back gracefully. On Windows, run inside **Git Bash**, since Claude Code executes the statusline through it.
...
Not a kitchen-sink widget dump. Every choice is deliberate:
